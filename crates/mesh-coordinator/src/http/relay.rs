...
pub fn route_uplink(
    ...
    if coordinator.relay().forward(&dst, downlink.clone(), hi_prio) {
        ...
        // Gated by the global proactive gate (like punch): with the gate OFF
        // (Stage-1 default) the coordinator emits NO RelayWake, so a fresh deploy
        // is byte-identical to today and sends nothing new to not-yet-upgraded
        // joiners. Convergence still works via the always-on joiner eager-kick;
        // the rendezvous wake is the accelerator that turns on with the gate.
        // `proactive_on()` is checked FIRST so the lookups + `take_wake_slot`
        // mutation are skipped entirely while the gate is off.
        if is_init
            && coordinator.proactive_on()
...
mod tests {
    ...
    async fn route_uplink_wakes_cold_destination_on_init() {
        let c = coordinator();
        c.set_proactive(true); // rendezvous wake is gated by the proactive gate
        let (a, _) = c
            .register(req(1, "a", "a", &["tag:user-a"]))
            .await
            .expect("a");
    ...
    #[tokio::test]
    async fn route_uplink_no_wake_when_gate_off() {
        let c = coordinator();
        // gate OFF (default) — deliberately do NOT call set_proactive.
        let _ = c
            .register(req(1, "a", "a", &["tag:user-a"]))
            .await
            .expect("a");
        ...
        let uplink = encode_relay_frame(&[2u8; 32], &[1u8, 0, 0, 0]);
        let out = route_uplink(&c, &[1u8; 32], &uplink);
        assert!(out.is_none(), "cold dst ⇒ spooled");
        assert!(
            sub.try_recv().is_err(),
            "gate OFF ⇒ NO RelayWake (Stage-1 is inert)"
        );
    }
